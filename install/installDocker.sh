...
install_systemd_service() {
  ...
  printf "  %b Installing systemd service file" "${INFO}"
   ...
   install -m 755 -o "${CYPHER_CYPNODE_USER}" -g "${CYPHER_CYPNODE_GROUP}" "/tmp/${CYPHER_CYPNODE_SYSTEMD_SERVICE}" "${SYSTEMD_SERVICE_PATH}${CYPHER_CYPNODE_SYSTEMD_SERVICE}"
  ...
  printf "  %b Enabling systemd service" "${INFO}"
   systemctl enable "${CYPHER_CYPNODE_SYSTEMD_SERVICE}" &> /dev/null
  printf "%b  %b Enabled systemd service\n" "${OVER}" "${TICK}"

  printf "  %b Starting systemd service" "${INFO}"
   systemctl start "${CYPHER_CYPNODE_SYSTEMD_SERVICE}" > /dev/null
...
install_launchd_service() {
  ...
  printf "  %b Installing launchd service file" "${INFO}"
   ...
   install -m 755 -o "${CYPHER_CYPNODE_USER}" -g "${CYPHER_CYPNODE_GROUP}" "/tmp/${CYPHER_CYPNODE_LAUNCHD_SERVICE}" "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}"
  ...
  printf "  %b Loading launchd service" "${INFO}"
   launchctl load "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}" &> /dev/null
  printf "%b  %b Loaded launchd service\n" "${OVER}" "${TICK}"

  printf "  %b Starting launchd service" "${INFO}"
   launchctl start "${CYPHER_CYPNODE_LAUNCHD_SERVICE}" > /dev/null
    ...
    if [ "${INIT}" = "systemd" ]; then
      if [ $(systemctl is-active "${CYPHER_CYPNODE_SYSTEMD_SERVICE}") = "active" ]; then
        ...
        printf "  %b Stopping systemd service" "${INFO}"
         systemctl stop "${CYPHER_CYPNODE_SYSTEMD_SERVICE}" >/dev/null
        printf "%b  %b Stopped systemd service\n" "${OVER}" "${TICK}"
      fi
    fi
  elif [ "${IS_MACOS}" = true ]; then
    if [ -f "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}" ]; then
      if [ $( launchctl list | grep "${CYPHER_CYPNODE_LAUNCHD_SERVICE}") ]; then
        printf "\n"
        printf "  %b Stopping launchd service" "${INFO}"
         launchctl stop "${CYPHER_CYPNODE_LAUNCHD_SERVICE}" >/dev/null
        printf "%b  %b Stopped systemd service\n" "${OVER}" "${TICK}"
      fi
    fi
  fi
...
user_create_linux() {
  echo groupadd "$1"
   groupadd -f "$1" >/dev/null
  if [ -f "/etc/arch-release" ]; then
     useradd --system --gid $(getent group "$1" | cut -d: -f3) --no-create-home "$2" >/dev/null
  else
     adduser --system --gid $(getent group "$1" | cut -d: -f3) --no-create-home "$2" >/dev/null
  fi
}
  ...
  for (( uid = 500;; --uid )) ; do
    if ! id -u $uid &>/dev/null; then
      if ! dscl /Local/Default -ls Groups gid | grep -q [^0-9]$uid\$ ; then
         dscl /Local/Default -create Groups/"$1" >/dev/null
         dscl /Local/Default -create Groups/"$1" Password \* >/dev/null
         dscl /Local/Default -create Groups/"$1" PrimaryGroupID $uid >/dev/null
         dscl /Local/Default -create Groups/"$1" RealName "$1" >/dev/null
         dscl /Local/Default -create Groups/"$1" RecordName _"$1" "$1" >/dev/null

         dscl /Local/Default -create Users/"$2" >/dev/null
         dscl /Local/Default -create Users/"$2" PrimaryGroupID $uid
         dscl /Local/Default -create Users/"$2" UniqueID $uid >/dev/null
        USER_CREATED=true
        break
      fi
    fi
...
install_archive() {
  ...
  printf "  %b Installing to %s" "${INFO}" "${CYPHER_CYPNODE_OPT_PATH}"
   mkdir -p "${CYPHER_CYPNODE_OPT_PATH}"
   cp -r "${CYPHER_CYPNODE_TMP_PATH}"* "${CYPHER_CYPNODE_OPT_PATH}"
   chmod -R 755 "${CYPHER_CYPNODE_OPT_PATH}"
   chown -R "${CYPHER_CYPNODE_USER}":"${CYPHER_CYPNODE_GROUP}" "${CYPHER_CYPNODE_OPT_PATH}"
  ...
  if [ "${IS_SKIP_CONFIG}" = true ]; then
    ...
    printf "  %b Running configuration util" "${INFO}"
     -u "${CYPHER_CYPNODE_USER}" "${CYPHER_CYPNODE_OPT_PATH}"cypnode --configure
      ...
      if [ -f "${SYSTEMD_SERVICE_PATH}${CYPHER_CYPNODE_SYSTEMD_SERVICE}" ]; then
        if [ $(systemctl is-enabled "${CYPHER_CYPNODE_SYSTEMD_SERVICE}") = "enabled" ]; then
          printf "  %b Disabling service" "${INFO}"
           systemctl disable "${CYPHER_CYPNODE_SYSTEMD_SERVICE}" >/dev/null 2>&1
          printf "%b  %b Disabled service\n" "${OVER}" "${TICK}"
        fi

        printf "  %b Removing service" "${INFO}"
         rm -f "${SYSTEMD_SERVICE_PATH}${CYPHER_CYPNODE_SYSTEMD_SERVICE}"
    ...
    if [ -f "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}" ]; then
      if [  launchctl list | grep "${CYPHER_CYPNODE_LAUNCHD_SERVICE}" ]; then
        printf "  %b Unloading service" "${INFO}"
         launchctl unload "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}" >/dev/null 2>&1
        printf "%b  %b Unloaded service\n" "${OVER}" "${TICK}"
      fi

      printf "  %b Removing service" "${INFO}"
       rm -f "${LAUNCHD_SERVICE_PATH}${CYPHER_CYPNODE_LAUNCHD_SERVICE}"
      printf "%b  %b Removed service\n" "${OVER}" "${TICK}"
    fi
  fi

   rm -rf "${CYPHER_CYPNODE_OPT_PATH}"

  if [ "${IS_LINUX}" = true ]; then
    if getent passwd "${CYPHER_CYPNODE_USER}" >/dev/null; then
      printf "  %b Removing user" "${INFO}"
       userdel "${CYPHER_CYPNODE_USER}" > /dev/null
    ...
    if dscl /Local/Default read /Users/"${CYPHER_CYPNODE_USER}" &>/dev/null; then
      printf "  %b Removing user" "${INFO}"
       dscl /Local/Default -delete /Users/"${CYPHER_CYPNODE_USER}" >/dev/null
       dscl /Local/Default -delete /Groups/"${CYPHER_CYPNODE_GROUP}" >/dev/null
      printf "%b  %b Removed user\n" "${OVER}" "${TICK}"
    fi
  fi
